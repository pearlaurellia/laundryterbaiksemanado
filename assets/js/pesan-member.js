...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formPesan.addEventListener('submit', async (e) => {
        e.preventDefault(); // Matikan fitur reload bawaan HTML form
        ...
        // Validasi client-side terakhir memanfaatkan file form-validation.js
        ...
        const labelOpsi = opsi === 'kurir' ? 'Kurir ke ' + (document.getElementById('inputKecamatan').value) : 'Ambil Sendiri';

        // Langkah Krusial: Buka WhatsApp SEBELUM melakukan Fetch (Bebas Popup Blocker)
        const pesanWa = encodeURIComponent(
            'Halo CleanCo, saya baru saja membuat pesanan laundry lewat website.\n' +
            'Nama Pelanggan: ' + memberNama + '\n' +
            'Paket Layanan: ' + window.namaLayananAktif + '\n' +
            'Metode Pengantaran: ' + labelOpsi
        );
        ...
        // Jalankan pengiriman data asinkron via Fetch POST
        try {
            ...
            // Jika baris database sukses ditulis, ledakkan popup sukses ke layar
            if (result.success) {
